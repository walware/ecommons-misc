...
public class PreferencesUtil {
	...
	private static <T> T getPrefValue(final IEclipsePreferences node, final Preference<T> key) {
		...
		case BOOLEAN:
			storedValue = node.getBoolean(key.getKey(), Preference.BOOLEAN_DEFAULT_VALUE);
			break;
		case INT:
			storedValue = node.getInt(key.getKey(), Preference.INT_DEFAULT_VALUE);
			break;
		case LONG:
			storedValue = node.getLong(key.getKey(), Preference.LONG_DEFAULT_VALUE);
			break;
		case DOUBLE:
			storedValue = node.getDouble(key.getKey(), Preference.DOUBLE_DEFAULT_VALUE);
			break;
		case FLOAT:
			storedValue = node.getFloat(key.getKey(), Preference.FLOAT_DEFAULT_VALUE);
			break;
		default:
