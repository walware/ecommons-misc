...
class BreadcrumbItem extends Item {
	 ...
	 */
	void refresh() {
		final String text= fLabelProvider.getText(getData());
		final Image image= fLabelProvider.getImage(getData());
		final String toolTip= fToolTipLabelProvider.getText(getData());
	...
	void setIsLastItem(final boolean isLast) {
		...
		final GridData data= (GridData) fContainer.getLayoutData();
