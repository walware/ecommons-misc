...
public class HTMLStripCharFilter extends BaseCharFilter {
  ...
  public static void main(final String[] args) throws IOException {
    final Reader in = new HTMLStripCharFilter(
  ...
  public HTMLStripCharFilter(final Reader source) {
    super( ((source instanceof CharStream) && ((CharStream) source).markSupported()) ?
            (CharStream) source : CharReader.get(new BufferedReader(source)) );
  }

  public HTMLStripCharFilter(final CharStream source, final Set<String> escapedTags){
    this(source);
    this.escapedTags = escapedTags;
  }

  public HTMLStripCharFilter(final CharStream source, final Set<String> escapedTags, final int readAheadLimit){
    this(source);
    this.escapedTags = escapedTags;
  ...
  private int next() throws IOException {
    ...
    if (len>0) {
      final int ch = pushed.charAt(len-1);
  ...
  private void push(final int ch) {
  ...
  private boolean isHex(final int ch) {
    return (ch>='0' && ch<='9') ||
           (ch>='A' && ch<='Z') ||
           (ch>='a' && ch<='z');
  }

  private boolean isAlpha(final int ch) {
...
***/
  ...
  private boolean isIdChar(final int ch) {
  ...
  private boolean isFirstIdChar(final int ch) {
  ...
  private int readEntity() throws IOException {
    ...
    if (ch==';') {
      ...
      final Character entityChar = entityTable.get(entity);
      if (entityChar!=null) {
  ...
  ***/
  ...
  private int readBang(final boolean inScript) throws IOException {
    // at this point, "<!" has been read
    final int ret = readComment(inScript);
  ...
  //
  private int readComment(final boolean inScript) throws IOException {
    // at this point "<!" has  been read
    int ch = next();
      if (ch!='-') {
      // not a comment
    ...
    while ((numRead - lastMark) < safeReadAheadLimit -3 ) {
      ...
      if (ch=='-') {
        ...
        push(ch);
        final int ret=readScriptString();
  ...
  private int readName(final boolean checkEscaped) throws IOException {
    final StringBuilder builder = (checkEscaped && escapedTags!=null) ? new StringBuilder() : null;
    int ch = next();
  ...
  ***/
    ...
    if ((numRead - lastMark < safeReadAheadLimit)) {
      ...
      final int quoteChar = nextSkipWS();
